...
@testset "inverse_frequencies" begin
    freqs = VectorTransformer.inverse_frequencies(100,8)
    @test length(freqs) == 5
...
@testset "rotate_half" begin
    ...
    r = VectorTransformer.rotate_half(m)
    @test r == [-3 -4 -5 1 2; -30 -40 -50 10 20]
end

@testset "frequencies" begin
    m = VectorTransformer.frequencies(100, 16, 40)
...
@testset "apply_rotary" begin
    ...
    r = VectorTransformer.apply_rotary(config, m)
